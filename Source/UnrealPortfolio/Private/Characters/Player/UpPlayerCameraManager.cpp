...
void AUpPlayerCameraManager::BeginPlay()
{
	...
	CustomController = Cast<AUpPlayerController>(GetOwningPlayerController());
}
...
void AUpPlayerCameraManager::AdjustCameraForCrouch(FTViewTarget& OutVT, const float DeltaTime)
{
	...
	{
		if (const auto MovementComponent = PossessedCharacter->GetCharacterMovement(); MovementComponent && MovementComponent->IsMovingOnGround())
		{
			...
			const auto DefaultCapsuleHalfHeight = PossessedCharacter->GetClass()->GetDefaultObject<ACharacter>()->GetCapsuleComponent()->GetScaledCapsuleHalfHeight();
